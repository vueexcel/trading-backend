...
const getTickerReturns = async (req, res) => {
    ...
    try {
        const returns = await analyticsData.calculateAllReturns(ticker, true, true, customRange, 1970);
...
const getIndexReturns = async (req, res) => {
    ...
    try {
        ...
        const payload = await analyticsData.calculateIndexReturns(indexValue, customRange, 1970);
        await setCache(cacheKey, payload, TICKER_DETAILS_CACHE_TTL_SECS);
        res.set('X-Cache-Hit', '0');
